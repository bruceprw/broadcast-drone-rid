...
// TODO modify characteristic permissions so that the OUTPUT characteristic requires encryption and INPUT requires encryption and authentication
...
// BT_GATT_PERM_WRITE_AUTHEN - requires encryption using authenticated link-key for write access.
// BT_GATT_PERM_READ_ENCRYPT - requires encryption for read access.
static struct bt_gatt_attr attrs[] = {
		BT_GATT_PRIMARY_SERVICE(BT_UUID_CUSTOM_SERVICE),
